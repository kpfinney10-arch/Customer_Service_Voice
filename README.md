...
- `GET /v1/tenants/:tenantId/diagnostics/sessions/:sessionId`
...
The repository includes `render.yaml` for an always-on Render web service and managed PostgreSQL database. Follow `docs/runbooks/render-cloud-deployment.md` to deploy, validate the temporary Render hostname, attach `voice.lanternbell.com`, and cut over from the local Cloudflare tunnel without interrupting the current working endpoint.
...
After each supported Telnyx webhook command batch, the service records a `PROVIDER_COMMANDS_EXECUTED` event in the call timeline. The event stores sanitized command names, status codes, dry-run flags, failure names, and short failure summaries so dry-run and controlled live tests can be audited later without storing Telnyx credentials.
