...
test("Twilio webhook route handles live hospice at-home transcript in one turn", async () => {
  await fetchText(
    ...
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-hospice-at-home-live-1",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-hospice-at-home-live-1/replay",
  );
...
test("Twilio webhook route handles hospice name and funeral home sentence breaks", async () => {
  await fetchText(
    ...
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-hospice-at-home-live-3",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const response = await fetchText(
    ...
    new URLSearchParams({
      CallSid: "twilio-call-http-hospice-at-home-live-3",
      SpeechResult:
        "Hi, this is Nurse Emily. Johnson with Gentle Care Hospice. I'm at the family's home with Mr. Robert Jones. He has passed away and the family has requested Smith Family. Funeral Home. My call back number is 214-639-5723. They're at 636 Commerce Avenue in Keller, Texas.",
      Confidence: "0.92",
    }),
    {
      apiKey: null,
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-hospice-at-home-live-3/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.caller_name, "Emily Johnson");
  assert.equal(replay.body.session.facts.caller_phone, "214-639-5723");
...
test("Twilio webhook route handles hospice article before titled decedent", async () => {
  await fetchText(
    "POST",
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-hospice-at-home-live-4",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-hospice-at-home-live-4/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.caller_name, "Emily Johnson");
  assert.equal(replay.body.session.facts.caller_phone, "214-639-5723");
...
test("Twilio webhook route handles live family authority-check call with spelling correction", async () => {
  await fetchText(
    "POST",
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-family-authority-live-1",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const caller = await fetchText(
    ...
    new URLSearchParams({
      CallSid: "twilio-call-http-family-authority-live-1",
      SpeechResult: "My name is Kyle, finny my phone number is 603-731-5845.",
      Confidence: "0.92",
    }),
    {
      apiKey: null,
  ...
  assert.match(caller.body, /spell/i);
  ...
  assert.match(spelling.body, /May I have the name of the person who passed away/i);
  ...
  const report = await fetchText(
    ...
    new URLSearchParams({
      CallSid: "twilio-call-http-family-authority-live-1",
      SpeechResult:
        "My Father Robert Jones passed away at home and we would like Smith Family Funeral Home to help us. He is at 6:36 Commerce Avenue, Keller Texas and I'm here with him now.",
      Confidence: "0.92",
    }),
    {
      apiKey: null,
  ...
  assert.match(report.body, /I am going to connect you with a funeral home team member now\./);
  assert.match(report.body, /<Dial /);

  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-family-authority-live-1/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.caller_name, "Kyle Finney");
  assert.equal(replay.body.session.facts.caller_phone, "603-731-5845");
  assert.equal(replay.body.session.facts.caller_relationship_to_decedent, "father");
  assert.equal(replay.body.session.facts.decedent_name, "Robert Jones");
  assert.equal(replay.body.session.facts.pickup_address, "636 Commerce Avenue Keller Texas");
  assert.equal(replay.body.session.facts.place_of_death_type, "residence");
  assert.equal(replay.body.session.facts.currently_with_decedent, true);
...
test("Twilio webhook route handles one-turn live officer residence report", async () => {
  await fetchText(
    ...
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-police-residence-live-3",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-police-residence-live-3/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.caller_name, "Officer Mendes");
  assert.equal(replay.body.session.facts.caller_phone, "817-632-4211");
...
test("Twilio webhook route handles latest live hospital release punctuation in one turn", async () => {
  await fetchText(
    ...
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-hospital-release-live-2",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-hospital-release-live-2/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
...
test("Twilio webhook route handles latest live hospital pickup-address phrasing", async () => {
  await fetchText(
    "POST",
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-hospital-release-live-4",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-hospital-release-live-4/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.caller_name, "David Carter");
  assert.equal(replay.body.session.facts.caller_phone, "214-639-5723");
...
test("Twilio webhook route handles live medical examiner false friends in one turn", async () => {
  await fetchText(
    "POST",
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-medical-examiner-live-2",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-medical-examiner-live-2/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.death_reported, true);
...
test("Twilio webhook route handles latest medical examiner release phrasing in one turn", async () => {
  ...
  assert.equal(replay.body.session.facts.currently_with_decedent, true);
  assert.equal(replay.body.session.facts.requested_funeral_home, "Smith Family Funeral Home");
  assert.equal(replay.body.snapshot.handoff.missingFacts.includes("currently_with_decedent"), false);
  assert.deepEqual(replay.body.snapshot.completedToolNames, [
    "crm.create_intake_lead",
    "dispatch.create_removal_request",
  ]);
...
test("Twilio webhook route handles live medical examiner dotted funeral home and pickup phrasing in one turn", async () => {
  await fetchText(
    "POST",
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-medical-examiner-live-4",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-medical-examiner-live-4/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.death_reported, true);
  assert.equal(replay.body.session.facts.caller_name, "Sarah Miller");
...
test("Twilio webhook route handles live bare tent county medical examiner phrase", async () => {
  await fetchText(
    "POST",
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-medical-examiner-live-bare-tent-county",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  assert.equal(opening.status, 200);
  assert.match(opening.body, /case number/i);
  assert.doesNotMatch(opening.body, /May I have the name of the person who passed away|Where is your loved one located right now/i);
  ...
  const location = await fetchText(
    ...
    new URLSearchParams({
      CallSid: "twilio-call-http-medical-examiner-live-bare-tent-county",
      SpeechResult: "He's at 200, Felix goes place in Fort Worth, Texas.",
      Confidence: "0.92",
    }),
    {
      apiKey: null,
  ...
  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-medical-examiner-live-bare-tent-county/replay",
  );
  assert.equal(replay.body.session.currentState, "ESCALATE");
  assert.equal(replay.body.session.facts.caller_name, "Maria Lopez");
  assert.equal(replay.body.session.facts.caller_phone, "603-731-5845");
  assert.equal(replay.body.session.facts.facility_name, "Tarrant County Medical Examiner's Office");
  assert.equal(replay.body.session.facts.decedent_name, "Robert Jones");
  assert.equal(replay.body.session.facts.crm_existing_case_reference, "2611232");
  assert.equal(replay.body.session.facts.pickup_address, "200 Feliks Gwozdz Place Fort Worth Texas");
  assert.equal(replay.body.session.facts.place_of_death_type, "medical_examiner");
  assert.equal(replay.body.session.facts.requested_funeral_home, "The Smith Family Funeral Home");
...
test("Twilio webhook route closes death-caller office-hours STT in one turn", async () => {
  await fetchText(
    "POST",
    "/v1/tenants/fh-demo/telephony/twilio/webhook",
    new URLSearchParams({
      CallSid: "twilio-call-http-family-office-hours-death-caller-1",
      From: "+16037315845",
      To: "+15559870000",
      CallStatus: "in-progress",
    }),
  ...
  const response = await fetchText(
    ...
    new URLSearchParams({
      CallSid: "twilio-call-http-family-office-hours-death-caller-1",
      SpeechResult:
        "Hi. My name is Kyle finny my call back. Number is 603-731-5845, my Father Robert finny is already in your care. This is not a new death caller, an emergency. I just wanted to ask whether I can drop off clothing tomorrow morning in what time the office opens.",
      Confidence: "0.92",
    }),
    {
      apiKey: null,
  ...
  assert.match(response.body, /follow up during office hours/);
  assert.match(response.body, /<Hangup\/>/);
  assert.doesNotMatch(response.body, /person who passed away|located right now|spell your name/i);
  assert.doesNotMatch(response.body, /<Gather /);
  assert.doesNotMatch(response.body, /<Dial/);

  const replay = await fetchJson(
    "GET",
    "/v1/tenants/fh-demo/first-call/sessions/twilio-call-http-family-office-hours-death-caller-1/replay",
  );
  assert.equal(replay.body.session.currentState, "WRAPUP");
  assert.equal(replay.body.session.intent, "service_schedule_question");
  assert.equal(replay.body.session.facts.reasonForCall, "service_schedule_question");
